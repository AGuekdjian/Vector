    ...
    await (
      await request.post("/api/customers", {
        data: {
          ...
          customerNumber: `SYS-C-${Date.now()}`,
          subscriberNumber: `AB-SYS-${Date.now()}`,
...
test("admin creates an order and its technician completes it with traceability", async ({
  ...
  const customerResponse = await page.request.post("/api/customers", {
    data: {
      ...
      firstName: "Juan",
      lastName: "Pérez",
      customerNumber: `JUAN-${Date.now()}`,
      subscriberNumber: `AB-JUAN-${Date.now()}`,
    ...
    await (
      await request.post("/api/customers", {
        data: {
          ...
          firstName: "Marta",
          lastName: "Resultado",
          customerNumber: `MARTA-${Date.now()}`,
          subscriberNumber: `AB-MARTA-${Date.now()}`,
